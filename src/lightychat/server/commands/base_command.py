...
class ServerCommand(ABC):
    ...
    @classmethod
    def discover(cls, package_path: str) -> Dict[str, "ServerCommand"]:
        ...
        for subclass in cls._get_all_subclasses(ServerCommand):
            if subclass.name:
    ...
    @staticmethod
    def _get_all_subclasses(base_class: Type["ServerCommand"]) -> list[Type["ServerCommand"]]:
        subclasses = base_class.__subclasses__()
        result: list[Type["ServerCommand"]] = []
        for sub in subclasses:
            result.append(sub)
            result.extend(ServerCommand._get_all_subclasses(sub))
        return result